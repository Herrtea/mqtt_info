...
class myThread(threading.Thread):
    def __init__(self,threadID,name,subTopic):
    ...
    def run(self):                   #把要执行的代码写到run函数里面 线程在创建后会直接运行run函数 
        mqtt_main(self.subtopicname,self.name)
...
def mqtt_main(subtopicName,client_id):
    ...
    mqtt.connect()
    mqtt.client.subscribe(topic=subtopicName,qos=0)
...
def listener():
    ...
    #启用线程开启客户端
    thread1 = myThread(1,client_id1,sub_topic1)
    thread1.start()
    
    thread1 = myThread(2,client_id2,sub_topic2)
    thread1.start()
    
    # generate ros Node
    # rospy.init_node('rtkinfo_listener', anonymous=True)

    # rospy.Subscriber("chatter", RTK, callback)
    # print('1')
